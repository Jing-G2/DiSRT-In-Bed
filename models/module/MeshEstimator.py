...
class MeshEstimator(nn.Module):
    def __init__(self, batch_size):
        ...
        self.N = batch_size
        shapedirs_f = (
            shapedirs_f.unsqueeze(0)
            .repeat(self.N, 1, 1, 1)
            .permute(0, 2, 1, 3)
            .unsqueeze(0)
        )  # 6890, 10, 3 -> 1, 6890, 10, 3 -> N, 6890, 10, 3 -> N, 10, 6890, 3 -> 1, N, 10, 6890, 3
        shapedirs_m = (
            shapedirs_m.unsqueeze(0)
            .repeat(self.N, 1, 1, 1)
            .permute(0, 2, 1, 3)
            .unsqueeze(0)
        )
        ...
        self.shapedirs = self.shapedirs.permute(1, 0, 2, 3, 4).view(
            self.N, 2, self.B * self.R * self.D
        )  # N X 2 X B X R X D ->  N, 2, B*R*D

        v_template_f = (
            v_template_f.unsqueeze(0).repeat(self.N, 1, 1).unsqueeze(0)
        )  # 6890, 3 -> 1, N, 6890, 3
        ...
        self.v_template = self.v_template.permute(1, 0, 2, 3).view(
            self.N, 2, self.R * self.D
        )  # N X 2 X R*D

        self.J_regressor = torch.cat(
            (
                J_regressor_f.unsqueeze(0).repeat(self.N, 1, 1).unsqueeze(0),
                J_regressor_m.unsqueeze(0).repeat(self.N, 1, 1).unsqueeze(0),
            ),
            0,
        )  # 2 X N X R X 24
        self.J_regressor = self.J_regressor.permute(1, 0, 2, 3).view(
            self.N, 2, self.R * 24
        )  # N X 2 X R*24

        self.posedirs = torch.cat(
            (
                posedirs_f.unsqueeze(0).repeat(self.N, 1, 1, 1).unsqueeze(0),  # f
                posedirs_m.unsqueeze(0).repeat(self.N, 1, 1, 1).unsqueeze(0),  # m
            ),
            0,
        )  # 2 X N X V X 3 X 207
        self.posedirs = self.posedirs.permute(1, 0, 2, 3, 4).view(
            self.N, 2, self.R_used * self.D * 207
        )  # N X 2 X V*D*207

        self.weights_repeat = torch.cat(
            (
                weights_f.unsqueeze(0).repeat(self.N, 1, 1).unsqueeze(0),  # f
                weights_m.unsqueeze(0).repeat(self.N, 1, 1).unsqueeze(0),  # m
            ),
            0,
        )  # 2 X N X V X 24
        self.weights_repeat = self.weights_repeat.permute(1, 0, 2, 3).view(
            self.N, 2, self.R_used * 24
        )  # N X 2 X V*24

    def forward(self, batch_gender, x, is_gt=False):
        """
        Input
        ------
        batch_gender : [B, 2] -> input gender
        x : [B, 88] -> output from the encoder model
            0-10    Body Shape (10)
            10-13   root xyz (3)
        ...
        if not is_gt:
            ...
            x[:, 19:91] *= 2.0 / torch.abs(self.bounds[0:72, 0] - self.bounds[0:72, 1])
            x[:, 19:91] = x[:, 19:91].tanh()
            x[:, 19:91] /= 2.0 / torch.abs(self.bounds[0:72, 0] - self.bounds[0:72, 1])
            x[:, 19:91] += torch.mean(self.bounds[0:72, 0:2], dim=1)

        # form rotation matrix
        joint_angles_matrix = angle_utils.batch_rodrigues(
            x[:, 19:91].view(-1, 24, 3).clone()
        ).view(-1, 24, 3, 3)
        ...
        # verts prediction
        shapedirs = torch.bmm(
            batch_gender, self.shapedirs[0:batch_size_infer, :, :]
        ).view(
            batch_size_infer, self.B, self.R * self.D
        )  # N, B, R*D
        betas_shapedirs_mult = (
            torch.bmm(use_betas.unsqueeze(1), shapedirs)
            .squeeze(1)
            .view(batch_size_infer, self.R, self.D)
        )  # N, R, D
        v_template = torch.bmm(
            batch_gender, self.v_template[0:batch_size_infer, :, :]
        ).view(
            batch_size_infer, self.R, self.D
        )  # N, R, D
        v_shaped = betas_shapedirs_mult + v_template  # N, R, D
        J_regressor_repeat = torch.bmm(
            batch_gender, self.J_regressor[0:batch_size_infer, :, :]
        ).view(
            batch_size_infer, self.R, 24
        )  # N, R, 24
        Jx = torch.bmm(v_shaped[:, :, 0].unsqueeze(1), J_regressor_repeat).squeeze(
            1
        )  # N, 24
        Jy = torch.bmm(v_shaped[:, :, 1].unsqueeze(1), J_regressor_repeat).squeeze(
            1
        )  # N, 24
        Jz = torch.bmm(v_shaped[:, :, 2].unsqueeze(1), J_regressor_repeat).squeeze(
            1
        )  # N, 24
        ...
        new_joint_locations_pred, A_pred = (
            angle_utils.batch_global_rigid_transformation(
                joint_angles_matrix,
                joint_locations_pred,
                self.parents,
                rotate_base=False,
            )
        )
        new_joint_locations_pred = (
            new_joint_locations_pred
            - joint_locations_pred[:, 0:1, :]
            + use_root_shift.unsqueeze(1)
        )

        v_shaped_red = v_shaped

        pose_feature = (
            (joint_angles_matrix[:, 1:, :, :])
            .sub(1.0, torch.eye(3).float().to(DEVICE))
            .view(-1, 207)
        )  # N, 207
        posedirs_repeat = (
            torch.bmm(batch_gender, self.posedirs[0:batch_size_infer, :, :])
            .view(batch_size_infer, self.R_used * self.D, 207)
            .permute(0, 2, 1)
        )  # N, 207, 30
        v_posed = torch.bmm(pose_feature.unsqueeze(1), posedirs_repeat).view(
            -1, self.R_used, self.D
        )  # N, 10, 3
        v_posed = v_posed.clone() + v_shaped_red
        weights_repeat = (
            torch.bmm(batch_gender, self.weights_repeat[0:batch_size_infer, :, :])
            .squeeze(1)
            .view(batch_size_infer, self.R_used, 24)
        )
        T = torch.bmm(weights_repeat, A_pred.view(batch_size_infer, 24, 16)).view(
            batch_size_infer, -1, 4, 4
        )
        v_posed_homo = torch.cat(
            [
                v_posed,
                torch.ones(batch_size_infer, v_posed.shape[1], 1).float().to(DEVICE),
            ],
            dim=2,
        )
        ...
        if not is_gt:
            verts_offset = (
                torch.Tensor(verts_red.clone().detach().cpu().numpy())
                .float()
                .to(DEVICE)
            )

            # joint locations
            targets_est_detached = (
                torch.Tensor(new_joint_locations_pred.clone().detach().cpu().numpy())
                .float()
                .to(DEVICE)
            )
            synth_joint_addressed = [3, 15, 4, 5, 7, 8, 18, 19, 20, 21]
            for real_joint in range(10):
                verts_offset[:, real_joint, :] = (
                    verts_offset[:, real_joint, :]
                    - targets_est_detached[:, synth_joint_addressed[real_joint], :]
                )

            for real_joint in range(10):
                new_joint_locations_pred[
                    :, synth_joint_addressed[real_joint], :
                ] = new_joint_locations_pred[
                    :, synth_joint_addressed[real_joint], :
                ].clone() + torch.add(
                    -1, 1
