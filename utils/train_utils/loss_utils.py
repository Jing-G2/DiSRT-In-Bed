...
from utils.constants import MAX_PMAP_REAL, FACES, DEVICE
...
def get_projected_pressure(verts, pmap):
    ...
    verts[:, :, 0] = (
        verts[:, :, 0] + verts[:, :, 0].floor().detach() - verts[:, :, 0].detach()
    )
    verts[:, :, 1] = (
        verts[:, :, 1] + verts[:, :, 1].floor().detach() - verts[:, :, 1].detach()
    )
    ...
    for i in range(64):
        for j in range(27):
            mask = (
                (verts[:, :, 0] == i) & (verts[:, :, 1] == j) & (verts[:, :, 2] >= 0)
            ).float()
            projected_pressure[:, 0, i, j] = (pmap * mask).sum(dim=-1) / (
                mask.sum(dim=-1) + EPS
            )
